...
export function baseSize(multiple = 1) {
  return tokens.baseSize * multiple
}
...
export function baseFontSize(increments = 0, withLineHeight = false) {
  const fontSize = tokens.baseFontSize + increments * tokens.fontSizeIncrement
  if (fontSize < tokens.minFontSize) {
    throw new Error(
      `The font size is too small! The minimum font-size to be used in the application is ${tokens.minFontSize}.`
    )
  }
  ...
  const lineHeight = fontSize * tokens.baseLineHeightRatio
